...
const SingleBlogCommentDel: FC<IRew> = ({ comments }) => {
  const { user } = useUser();

  const isAdmin = Array.isArray(user?.role) && user.role.includes("admin");
  const isProductOwner = user?.sub === comments.user_id;

  const handleDelete = async () => {
    await deleteBlogComment(comments.id);
  };
...
export default SingleBlogCommentDel;
