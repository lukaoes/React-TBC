...
import SingleBlogRemoveModal from "./singleBlogRemoveModal";

interface IProd {
  blogPost: any;
}

const SingleBlogRemoveButton: FC<IProd> = ({ blogPost }) => {
  const [isEditModalOpen, setEditModalOpen] = useState(false);
  const { user } = useUser();

  const isAdmin = Array.isArray(user?.role) && user.role.includes("admin");
  const isProductOwner = user?.sub === blogPost.user_id;
  ...
  return (
    <div>
      ...
      <button
        ...
        className="flex items-center gap-1 font-bold text-[14px] text-[#F32013]"
      >
        <svg
          fill="#F32013"
          version="1.1"
...
export default SingleBlogRemoveButton;
