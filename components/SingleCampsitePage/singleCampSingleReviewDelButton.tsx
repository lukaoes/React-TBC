...
import { deleteCampsiteReview } from "../../actions";

interface IRew {
  review: Review;
}

const SingleCampSingleReviewDelButton: FC<IRew> = ({ review }) => {
  const { user } = useUser();

  const isAdmin = Array.isArray(user?.role) && user.role.includes("admin");
  const isProductOwner = user?.sub === review.user_id;

  const handleDelete = async () => {
    await deleteCampsiteReview(review.id);
  };
...
export default SingleCampSingleReviewDelButton;
